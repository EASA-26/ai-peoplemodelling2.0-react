...
import { UploadCloud, Trash2, FileText, Users, Briefcase, FileBadge, RefreshCw, Database } from "lucide-react";
...
export default function DataManagementPage() {
    ...
    const fetchItems = async (tab: string) => {
        setLoading(true);
        try {
            const res = await axios.get(`/api/uploads/${endpointMap[tab]}`);
            ...
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                ...
                <div className="p-6 border-b border-slate-100 bg-white flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    ...
                    <div className="flex items-center gap-3 flex-wrap">
                        <label className="cursor-pointer bg-blue-50 text-blue-700 hover:bg-blue-100 font-medium py-2 px-4 rounded-lg flex items-center gap-2 transition-colors border border-blue-200 text-sm">
                        ...
                        <button onClick={handleDeleteAll} className="bg-slate-50 text-red-600 hover:bg-red-50 hover:border-red-200 font-medium py-2 px-4 rounded-lg flex items-center gap-2 transition-colors border border-slate-200 text-sm">
